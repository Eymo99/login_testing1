...
function logActivity($message, $ip,$os, $loginCount){
    $file = fopen("log.txt", "a");
    date_default_timezone_set('America/Mexico_City');
    $timestamp = date("Y-m-d H:i:s");
    $os = PHP_OS;
    $logMessage = $timestamp . " - " . $message . " - Direccion IP: " . $ip .  " - Sistema Operativo: " . $os . " - Contador de Inicios de Sesion: " . $loginCount . PHP_EOL;
                    ...
                    if(mysqli_stmt_fetch($stmt)) {
                        if (password_verify($password, $hashed_password)) {
                            ...
                            if($update_stmt = mysqli_prepare($link, $update_sql)){
                                ...
                                $os = PHP_OS;
                                logActivity("El usuario $username ha iniciado sesión.", $client_ip, $os, $login_count);
                                header("location: welcome.php");
                                exit;
                            } else {
